...
As the algorithm runs and the shortest paths to vertexes are calculated the display is updated. 
Egdes change turn red when relaxed and green when a shorter path is found. 
...
- Edge.java: Contains Edge class which represents edges in the graph. 
- adjacencyList.java: Contains the graph represented by an adjacencyList that holds Edge objects, and naive algorithm. 

To Run:
1. Run the main class (App)
2. A window will open displaying the graph.
3. The Bellman-Ford algorithm will start automatically. 
   Relaxed edges will be highlighted in red, then green if it contains a shorter path to a node. 
   Labels beneath the node update showing the shortest known distances to the node, taken from the shortestPath arr. 
